...
export function WeatherComparison({ data }: WeatherComparisonProps) {
  ...
  const selectedItem = data.find((item) => item.date === selectedDate) || data[0]

  const radarData = [
    { metric: "Temperature", value: normalizeValue(selectedItem.temperature_c, "temperature_c") },
    { metric: "Humidity", value: normalizeValue(selectedItem.humidity_percent, "humidity_percent") },
    { metric: "UV Index", value: normalizeValue(selectedItem.uv_index, "uv_index") },
    { metric: "Precipitation", value: normalizeValue(selectedItem.precipitation_mm, "precipitation_mm") },
    { metric: "Wind Speed", value: normalizeValue(selectedItem.wind_speed_kmh, "wind_speed_kmh") },
  ]
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="text-sm font-medium">{selectedItem.temperature_c}°C</div>

            <div className="text-sm">Humidity:</div>
            <div className="text-sm font-medium">{selectedItem.humidity_percent}%</div>
            ...
            <div className="text-sm font-medium">{selectedItem.precipitation_mm} mm</div>

            <div className="text-sm">Wind Speed:</div>
            <div className="text-sm font-medium">{selectedItem.wind_speed_kmh} km/h</div>
          </div>
        </div>
